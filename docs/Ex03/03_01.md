...
1. Select **Endpoints**.
    ...
    {: .note } On the Endpoints page, this will default to the **General** > **Advanced features** menu item.
...
1. On the top bar, select **Save**.
    ...
    {: .warning }Please wait 5-10 minutes for the connection to be established between MDE and Intune.
...
1. On the top bar, select **Run Script**.
    ...
    {: .note } This performs a local health check for Microsoft Defender for Endpoint (MDE). It gathers and displays the device's onboarding state, organization info, and security configuration (Tamper Protection, PUA protection, Real-time protection, running mode, and MDE services like Sense and WinDefend).
