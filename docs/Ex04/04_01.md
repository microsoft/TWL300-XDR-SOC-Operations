...
{: .note }
> **Optional Task**: At this point, the Engineering team could check the status of the MDI sensors by running a script.
...
1. Run the following:  
    ...
    {: .note }
    > Checks each domain controller for MDI sensor and updater services, reporting reachability and service status.
    >
...
1. Select the **Service/detection sources** filter, select **Microsoft Defender for Identity** filter, then select **Apply**.
    ...
    {: .warning }
    > If you do not see the filter, you can proceed. If you do, you still may not see any results at the moment. 
...
1. Run the following KQL query:  
    ...
    {: .note }
    > Expect zero rows until sensors exist for this and the following two queries.
...
1. Run the following KQL query:
    ...
    {: .warning }
    > If you see an error, the table has not been provisioned yet. You can proceed.