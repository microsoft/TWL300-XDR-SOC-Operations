...
1. Ensure **Microsoft 365** is connected.  
    ...
    {: .note } This activates the Cloud Apps engine. 
...
1. Under **Enable policy,** select **On**.
    ...
    {: .important } In production, you should always test policies in **Report-only** first.

1. Select **Create**.

    {: .warning }Please wait 5 minutes for the policy to propagate before proceeding.
...
1. Sign in with **Lab User One**:
    ...
    {: .note } This is to test the new policy, which will pick up **Lab User One**'s sign in.
...
1. Below the tabs and **Filters**, select **Create policy** > **Session policy**.
    ...
    {: .note } If you see an error at the top saying **Conditional Access not found**, but the fields are not grayed out, you can proceed. 

    {: .warning }If the fields are grayed out, wait a minute and try again. It should pick up your sign in with **Lab User One** to Outlook fairly quickly.
...
1. At the bottom of the page, select **Create**.
    ...
    {: .important } Session policies in Cloud Apps are enforced by the reverse proxy behind **Conditional Access App Control**.  
...
1. Run the following KQL query:
    ...
    {: .important } The query lists all user actions from the past 24 hours that passed through a Defender for Cloud Apps session or access controls, helping you confirm which sessions were governed by Conditional Access App Control.
    >
