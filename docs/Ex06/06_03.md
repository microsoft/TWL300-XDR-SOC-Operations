...
1. Run the following KQL query:
    ...
    {: .note } This uses mock data, but is designed to hunt for logon activity targeting "High-Value Assets" (HVAs) - like critical servers - over the past 7 days.
...
1. Run the following KQL query to get the OAuth blast radius:
    ...
    {: .important } Review the results and export the CSVs for evidence.